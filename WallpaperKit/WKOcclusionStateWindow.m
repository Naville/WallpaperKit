...
+ (instancetype)sharedInstance {
        dispatch_once(&onceToken, ^{
          ...
          NSRect windowRect = NSMakeRect(
              rawSize.size.width / 20, rawSize.size.height / 20,
              11 * rawSize.size.width / 12, 11 * rawSize.size.height / 12);
          // CGRect windowRect=[NSScreen mainScreen].visibleFrame;
          sI = [[WKOcclusionStateWindow alloc]
...
- (instancetype)initWithContentRect:(NSRect)contentRect
        ...
        NSTrackingArea *foo =
            [[NSTrackingArea alloc] initWithRect:self.contentView.frame
...
- (void)mouseEntered:(NSEvent *)event {
        NSUInteger currentID =
            [[WKDesktopManager sharedInstance] currentSpaceID];
        WKDesktop *wk =
            [[WKDesktopManager sharedInstance] desktopForSpaceID:currentID];
...
- (void)refreshOSState {
        ...
        NSDictionary *UserInfo = @{
                @"CurrentSpaceID" : [NSNumber
                    numberWithInteger:[[WKDesktopManager sharedInstance]
                                          currentSpaceID]],
                @"Visibility" : [NSNumber numberWithBool:isVisible]
        };
        [[NSNotificationCenter defaultCenter]
            postNotificationName:OStateChangeNotificationName
                          object:nil
                        userInfo:UserInfo];
}
