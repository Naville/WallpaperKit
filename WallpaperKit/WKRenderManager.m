...
- (NSDictionary *)randomRender {
        if ([_renderList count] == 0) { // This is the last renderer
                @throw [NSException exceptionWithName:NSInvalidArgumentException
                                               reason:@"No Available Render"
                                             userInfo:nil];
        }
        return
            [self.renderList objectAtIndex:arc4random() % [_renderList count]];
}
+ (NSArray *)CovertRenders:(NSMutableArray<NSDictionary *> *)renderList
        ...
        @autoreleasepool {
                ...
                for (NSDictionary *dict in renderList) {
                        ...
                        NSString *libPath = [dict
                            objectForKey:@"LibraryPath"]; // Load External
                                                          // Libraries if exists
                        if (libPath != nil &&
                            [libPath respondsToSelector:@selector
                                     (isEqualToString:)]) {
                                dlopen(libPath.UTF8String,
                                       RTLD_NOW | RTLD_GLOBAL);
                                [[NSBundle bundleWithPath:libPath] load];
                        }
                        if ([render respondsToSelector:@selector
                                    (isEqualToString:)]) {
                        ...
                        [retArray
                            addObject:[cls convertArgument:dict Operation:op]];
                }
                return retArray;
        }
}
- (void)collectFromWallpaperEnginePath:(NSString *)RootPath {
        @autoreleasepool {
                NSError *err;
                for (
                    NSURL *path in [[NSFileManager defaultManager]
                          contentsOfDirectoryAtURL:[NSURL
                                                       fileURLWithPath:RootPath]
                        includingPropertiesForKeys:
                            [NSArray arrayWithObject:NSURLNameKey]
                                           options:
                        ...
                        NSMutableDictionary *ProjectInfo = [NSJSONSerialization
                            JSONObjectWithData:projectData
                                       options:NSJSONReadingMutableContainers
                                         error:nil];

                        NSString *projectType = [(NSString *)[ProjectInfo
                            objectForKey:@"type"] lowercaseString];

                        NSURL *filePath =
                            [path URLByAppendingPathComponent:
                                      [ProjectInfo objectForKey:@"file"]];
                        [ProjectInfo setObject:filePath.path forKey:@"Path"];

                        if ([projectType isEqualToString:@"video"]) {
                                NSDictionary *ConvertedProjectInfo =
                                    [[WKVideoPlugin class]
                                        convertArgument:ProjectInfo
                                              Operation:FROMJSON];
                                [self.renderList
                                    addObject:ConvertedProjectInfo];

                        } else if ([projectType isEqualToString:@"web"]) {
                                NSMutableDictionary *ConvertedProjectInfo =
                                    [[WKWebpagePlugin class]
                                        convertArgument:ProjectInfo
                                              Operation:FROMJSON];
                                [ConvertedProjectInfo setObject:path
                                                         forKey:@"BaseURL"];
                                [self.renderList
                                    addObject:ConvertedProjectInfo];
                        } else {
                                NSLog(@"WallpaperEngine Project Type:%@ "
                                      @"Unsupported",
                                      [ProjectInfo objectForKey:@"type"]);
                        }
                }
                if (err != nil) {
        ...
        @autoreleasepool {
                ...
                NSString *libPath =
                    [dict objectForKey:@"LibraryPath"]; // Load External
                                                        // Libraries if exists
                if (libPath != nil &&
                    [libPath respondsToSelector:@selector(isEqualToString:)]) {
                ...
                if ([render respondsToSelector:@selector(isEqualToString:)]) {
                        ...
                        if (class_conformsToProtocol(
                                cls, @protocol(WKRenderProtocal))) {
                                NSDictionary *foo =
                                    [cls convertArgument:dict
                                               Operation:FROMJSON];
