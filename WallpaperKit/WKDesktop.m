...
#define ALLKEYBOARDMOUSEEVENTS                                                 \
        NSEventMaskKeyDown | NSEventMaskKeyUp | NSEventMaskLeftMouseDown |     \
            NSEventMaskRightMouseDown | NSEventMaskLeftMouseUp |               \
            ...
            NSEventMaskCursorUpdate | NSEventMaskScrollWheel |                 \
...
- (instancetype)initWithContentRect:(NSRect)contentRect
        ...
        [[NSNotificationCenter defaultCenter]
            addObserver:self
               selector:@selector(handleOSChange:)
                   name:OStateChangeNotificationName
                 object:nil];
        return self;
}
- (void)renderWithEngine:(nonnull Class)renderEngine
           withArguments:(NSDictionary *)args {

        if (renderEngine == nil) {
                @throw [NSException exceptionWithName:NSInvalidArgumentException
                                               reason:@"RenderEngine is null"
                                             userInfo:nil];
        }
        if (![renderEngine conformsToProtocol:@protocol(WKRenderProtocal)] ||
            ![renderEngine isSubclassOfClass:[NSView class]]) {
                @throw [NSException
                    exceptionWithName:NSInvalidArgumentException
                               reason:[NSString
                                          stringWithFormat:
                                              @"%@ is not a valid "
                                              @"WKRenderProtocal class",
                                              NSStringFromClass(renderEngine)]
                             userInfo:nil];
        }
        NSView<WKRenderProtocal> *foo =
            [(NSView<WKRenderProtocal> *)[renderEngine alloc]
                initWithWindow:self
                  andArguments:args];
        [self addView:foo];
}
- (void)handleOSChange:(NSNotification *)notification {
        BOOL isVisible =
            [[notification.userInfo objectForKey:@"Visibility"] boolValue];
        NSNumber *newSpaceID =
            [notification.userInfo objectForKey:@"CurrentSpaceID"];
...
- (void)play {
        ...
        self->eventMonitor = [NSEvent
            addGlobalMonitorForEventsMatchingMask:ALLKEYBOARDMOUSEEVENTS
