...
- (instancetype)init {
        ...
        [HTTPHeaders setObject:@"Mozilla/5.0 (Macintosh; Intel Mac OS X "
        ...
        [HTTPHeaders setObject:@"http://music.163.com/search/"
                        forKey:@"Referer"];
        return self;
}
- (NSDictionary *)searchSongInfo:(NSString *)s {
        // NSString* s=[NSString stringWithFormat:@"%@ %@",[Info
        // objectForKey:@"Song"],[Info objectForKey:@"Artist"]];
        NSDictionary *Params = @{
                @"s" : s,
                @"type" : @1,
                @"offset" : @0,
        ...
        NSData *POSTData =
            [self POST:@"http://music.163.com/api/search/get" Params:Params];
        return
            [NSJSONSerialization JSONObjectWithData:POSTData
...
- (nullable NSDictionary *)searchLyricForSongInfo:(nonnull NSDictionary *)Info {
        @autoreleasepool {
                ...
                for (NSString *Rule in [[WKConfigurationManager sharedInstance]
                            ...
                            stringByReplacingOccurrencesOfString:@"%SONG%"
                                                      withString:
                                                          [Info objectForKey:
                                                                    @"Song"]];
                        S = [S
                            stringByReplacingOccurrencesOfString:@"%ALBUM%"
                                                      withString:
                                                          [Info objectForKey:
                                                                    @"Album"]];
                        S = [S
                ...
                NSMutableArray *validSongs =
                    [self validSongs:allSongs Info:Info];
                NSNumber *SongID = validSongs[0][@"id"];
                NSString *LRCURL = [NSString
                    stringWithFormat:@"http://music.163.com/api/song/"
                                     @"lyric?os=osx&id=%@&lv=-1&kv=-1&tv=-1",
                                     SongID];
                NSDictionary *LRCInfo = [NSJSONSerialization
                    JSONObjectWithData:[NSData dataWithContentsOfURL:
                                                   [NSURL URLWithString:LRCURL]]
                               options:NSJSONReadingMutableLeaves
                                 error:nil];
                ...
                if (![Translated isEqual:[NSNull null]] &&
                    Translated.length > 0) {
        ...
        @autoreleasepool {
                ...
                for (NSDictionary *item in allSongs) {
                        ...
                        for (NSDictionary *ArtistInfo in item[@"artists"]) {
                                if ([(NSString *)ArtistInfo[@"name"]
                                        containsString:Info[@"Artist"]] ||
                                    [(NSString *)ArtistInfo[@"name"]
                                        containsString:Info[@"AlbumArtist"]] ||
                                    [Info[@"Artist"]
                                        containsString:(NSString *)ArtistInfo
                                                           [@"name"]] ||
                                    [Info[@"AlbumArtist"]
                                        containsString:(NSString *)ArtistInfo
                                                           [@"name"]]) {
                        ...
                        if (([(NSString *)item[@"name"]
                                 containsString:[Info objectForKey:@"Song"]] ||
                             [(NSString *)[Info objectForKey:@"Song"]
                                 containsString:(NSString *)item[@"name"]]) &&
                            ValidArtist) {
...
- (NSData *)POST:(NSString *)URL Params:(id)Params {
        ...
        NSError *err;
        NSData *retVal = [NSURLConnection sendSynchronousRequest:request
...
- (NSString *)AESEncrypt:(NSString *)Input Key:(NSString *)key {
        ...
        [@"0102030405060708" getCString:ivPtr
        ...
        CCCryptorStatus cryptorStatus = CCCrypt(
            kCCEncrypt, kCCAlgorithmAES128, 0, keyPtr, kCCKeySizeAES128, ivPtr,
            [dat bytes], [dat length], buffer, bufferSize, &numBytesEncrypted);
...
- (NSString *)RSAEncrypt:(NSString *)Input {
        ...
        for (NSUInteger i = 0; i < len; i++) {
                [hexString
                    appendString:[NSString stringWithFormat:@"%x", chars[i]]];
        }
        free(chars);
        ...
        mpz_set_str(ModulusHex,
                    "00e0b509f6259df8642dbc35662901477df22677ec152b5ff68ace615b"
                    "b7b725152b3ab17a876aea8a5aa76d2e417629ec4ee341f56135fccf69"
                    "5280104e0312ecbda92557c93870114af6c9d05c4f7f0c3685b7a46bee"
                    "255932575cce10b424d813cfe4875d3e82047b97ddef52741d546b8e28"
...
- (NSDictionary *)EncryptParams:(NSDictionary *)Input {
        NSString *JSON = [[NSString alloc]
            initWithData:[NSJSONSerialization
                             dataWithJSONObject:Input
                                        options:NSJSONWritingPrettyPrinted
                                          error:nil]
                encoding:NSUTF8StringEncoding];
        NSString *sKey = [self createSecretKey:16];
        NSString *EncryptedText =
            [self AESEncrypt:JSON Key:@"0CoJUm6Qyw8W8jud"];
