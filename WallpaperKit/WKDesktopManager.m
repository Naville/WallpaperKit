...
extern CFArrayRef CGSCopySpaces(const CGSConnectionID cid,
                                CGSSpaceSelector type);
...
@implementation WKDesktopManager {
        ...
        WKOcclusionStateWindow *DummyWindow; // Dummy Transparent Window at
                                             // kCGDesktopIconWindowLevel+1 for
                                             // OcclusionState Observe
}
...
- (void)DisplayDesktop:(WKDesktop *)wk {
        ...
        for (id key in self.windows.allKeys) {
                ...
                if ([currentDesktop
                        isEqualTo:wk]) { // Ignore next space's WKDesktop
                        continue;
                }
                if (currentDesktop.mainView.requiresConsistentAccess == YES &&
                    wk.mainView.requiresConsistentAccess == NO) {
                        // Old view needs consistent access while the new one
...
- (NSUInteger)currentSpaceID {
        CFArrayRef spaces =
            CGSCopySpaces(CGSDefaultConnection, kCGSSpaceCurrent);
        // CFArrayRef spaces = CGSCopySpaces(CGSDefaultConnection,
        // kCGSSpaceAll);
        ...
        for (ii = count - 1; ii >= 0; ii--) {
                CGSSpace spaceId =
                    [(__bridge id)CFArrayGetValueAtIndex(spaces, ii) intValue];
                if (CGSSpaceGetType(CGSDefaultConnection, spaceId) ==
                    kCGSSpaceSystem)
                        continue;
                CFRelease(spaces);
                return spaceId;
...
- (void)discardSpaceID:(NSUInteger)spaceID {
        if ([self.windows.allKeys
                containsObject:[NSNumber numberWithInteger:spaceID]]) {

                [[self.windows
                    objectForKey:[NSNumber numberWithInteger:spaceID]] close];
                [self->_windows
                    removeObjectForKey:[NSNumber numberWithInteger:spaceID]];
        }
}
...
- (WKDesktop *)createDesktopWithSpaceID:(NSUInteger)SpaceID
                              andRender:(NSDictionary *)render {
        WKDesktop *wk = [[WKDesktop alloc]
            initWithContentRect:CGDisplayBounds(CGMainDisplayID())
                      styleMask:NSWindowStyleMaskBorderless
                        backing:NSBackingStoreBuffered
                          defer:NO];
        if (render == nil || ![render.allKeys containsObject:@"Render"]) {
                @throw [NSException exceptionWithName:NSInvalidArgumentException
                                               reason:@"Render Invalid"
                                             userInfo:render];
        }
        [wk renderWithEngine:[render objectForKey:@"Render"]
               withArguments:render];
        wk.spaceID = [NSNumber numberWithInteger:SpaceID];
        [self->_windows setObject:wk
                           forKey:[NSNumber numberWithInteger:SpaceID]];
