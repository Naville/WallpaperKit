...
- (void)viewDidLoad {
        ...
        self.view.window.collectionBehavior =
...
- (IBAction)loadActiveSpace:(id)sender {
        WKDesktop *render =
            [self->wkdm createDesktopWithSpaceID:[self->wkdm currentSpaceID]
                                       andRender:[self->wkrm randomRender]];
...
- (IBAction)chooseRenderForCurrentDesktop:(id)sender {
        ...
        WKDesktop *wk =
            [self->wkdm desktopForSpaceID:[self->wkdm currentSpaceID]];
        if (wk == nil) {
                wk = [self->wkdm
                    createDesktopWithSpaceID:[self->wkdm currentSpaceID]
                                   andRender:[self->wkrm.renderList
                                                 objectAtIndex:index]];
        } else {
...
- (IBAction)SaveCurrentRender:(id)sender {
        ...
        NSArray *converted =
            [WKRenderManager CovertRenders:self->wkrm.renderList
                                 operation:TOJSON];
        NSError *err;
        NSData *convertedData =
            [NSJSONSerialization dataWithJSONObject:converted
...
- (NSString *)tableView:(NSTableView *)tableView
    objectValueForTableColumn:(nullable NSTableColumn *)tableColumn
        ...
        NSDictionary *convertedJSON =
            [cls convertArgument:arg Operation:TOJSON];
        return [[[NSString alloc]
            initWithData:[NSJSONSerialization
                             dataWithJSONObject:convertedJSON
                                        options:NSJSONWritingPrettyPrinted
                                          error:nil]
...
- (void)tableView:(NSTableView *)tableView
    ...
    forTableColumn:(nullable NSTableColumn *)tableColumn
        ...
        NSDictionary *dict = [NSJSONSerialization
            JSONObjectWithData:[object dataUsingEncoding:NSUTF8StringEncoding]
                       options:NSJSONReadingMutableLeaves
                         error:nil];
        Class cls = NSClassFromString([dict objectForKey:@"Render"]);
        NSDictionary *convertedJSON =
            [cls convertArgument:dict Operation:FROMJSON];
...
- (void)handleWorkspaceChange:(NSNotification *)notification {
        ...
        [self
            setTitle:[[self->wkdm desktopForSpaceID:[self->wkdm currentSpaceID]]
                         description]];
        NSUserNotification *NSUN = [NSUserNotification new];
        NSUN.title = @"WorkSpace Changed";
        NSUN.informativeText = self.title;
        [[NSUserNotificationCenter defaultUserNotificationCenter]
            deliverNotification:NSUN];
}
@end
